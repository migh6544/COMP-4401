...
def listOfFloats(values):
    ...
    - list: A list containing the values successfully cast to floats.
            Values that cannot be cast or are missing will be ignored.
    """
    ...
    for value in values:
        ...
        try:
            # Attempt to cast the value to a float and append to the list
...
def realStateAnalysis(location):
    """
    Analyze real estate data for a specified location.

    Parameters:
    - location (str): The name of the location.

    Outputs:
    - Writes the analysis results to 'realStateAnalysis.csv'.
    """

    # Ask user for valid file names and handle exceptions
    while True:
        try:
            buyPricesFileName = input(f"Enter the filename for buy prices in {location} (must end in .csv): ")
            if not buyPricesFileName.endswith('.csv'):
                raise ValueError("Filename must end with .csv")
            buyPrices = inputData(buyPricesFileName)
            break
        except FileNotFoundError:
            print("File not found. Please try again.")

    # Repeat for living space and rent prices
    while True:
        try:
            livingSpaceFileName = input(f"Enter the filename for living space in {location} (must end in .csv): ")
            if not livingSpaceFileName.endswith('.csv'):
                raise ValueError("Filename must end with .csv")
            livingSpace = inputData(livingSpaceFileName)
            break
        except FileNotFoundError:
            print("File not found. Please try again.")
    ...
    while True:
        try:
            rentPricesFileName = input(f"Enter the filename for rent prices in {location} (must end in .csv): ")
            if not rentPricesFileName.endswith('.csv'):
                raise ValueError("Filename must end with .csv")
            rentPrices = inputData(rentPricesFileName)
            break
        except FileNotFoundError:
            print("File not found. Please try again.")

    # Compute averages and perform conversions
    avgBuyPrice = eurosToDollars(computeAverage(listOfFloats(buyPrices)))
    avgRentPrice = eurosToDollars(computeAverage(listOfFloats(rentPrices)))
...
# Get user input for the first location and call the function
firstLocation = input("Please enter the first location (Madrid or Netherlands): ")
realStateAnalysis(firstLocation)

# Determine the second location and call the function
secondLocation = "Netherlands" if firstLocation == "Madrid" else "Madrid"
print(f"Please provide the files for {secondLocation}.")
realStateAnalysis(secondLocation)
...
def main():
    ...
    # Test the realStateAnalysis function
    print("Testing realStateAnalysis function...")
    firstLocation = input("Please enter the first location (Madrid or Netherlands): ")
    realStateAnalysis(firstLocation)
